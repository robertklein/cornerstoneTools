...
const logger = getLogger('tools:annotation:FreehandRoiToolOrig');
 ...
 */
export default class FreehandRoiToolOrig extends BaseAnnotationTool {
  constructor(props = {}) {
    const defaultProps = {
      name: 'FreehandRoiToolOrig',
  ...
  getPixels(image, x, y, width, height) {
    x = Math.round(x);
    y = Math.round(y);
    const storedPixels = [];
    let index = 0;
  ...
  calculateStatsOffscreen(image, data) {
    // Define variables for the area and mean/standard deviation
    let meanStdDev, meanStdDevSUV;
    ...
    const colPixelSpacing = image.columnPixelSpacing;
    const rowPixelSpacing = image.rowPixelSpacing;
